        ...
        while True:
            if command_prompt_matched:
                try:
                    ...
                    display.display('-- receive data 2: %s--' % data, log_only=True)
                    signal.alarm(0)
                ...
                sleep(1)
                data = self._ssh_shell.read_nonblocking(256)
                display.display('-- receive data 1: %s--' % data, log_only=True)
        ...
        if not is_error_message:
            for regex in self._terminal.terminal_stdout_re:
                ...
                if match:
                    ...
                    display.display('-- _find_prompt error self._matched_pattern: %s--' % self._matched_pattern, log_only=True)
